...
import edu.wm.cs.cs301.amazebyyunwoonoh.falstad.BSPBranch;
import edu.wm.cs.cs301.amazebyyunwoonoh.falstad.BSPLeaf;
import edu.wm.cs.cs301.amazebyyunwoonoh.falstad.BSPNode;
import edu.wm.cs.cs301.amazebyyunwoonoh.falstad.CardinalDirection;
import edu.wm.cs.cs301.amazebyyunwoonoh.falstad.Cells;
import edu.wm.cs.cs301.amazebyyunwoonoh.falstad.MazeBuilder;
import edu.wm.cs.cs301.amazebyyunwoonoh.falstad.Seg;
import edu.wm.cs.cs301.amazebyyunwoonoh.falstad.Wall;
...
public class FirstPersonDrawer extends DefaultViewer {
	 ...
	 */
	@Override
	public void redraw(MazePanel panel, StateGUI state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		//by calling this method, the work in the comment is done.
		//panel.create2DGraphics();
		/*
	...
	private boolean boundingBoxIsVisible(BSPNode b) {
		...
		// constraint: x1 <= x2
		int[] point = new int[]{x1, x2};
        //panel.setPoint(x1,x2);
		//Point p = new Point(x1, x2);
		return (rset.intersect(point));
	}
	 ...
	 * param before refactoring: Seg seg 
	 */
	private void drawSegment(Seg seg) {
		...
		int x1i = x1;
		int xd = x2-x1;
		panel.setterWithParmColor(seg.panelGetter().RGBGetter());
		...
		while (x1i <= x2) {
			...
			//panel.setPoint(x1i, x2);
			int[] point = new int[]{x1i, x2};

			if (!rset.intersect(point))
				break;
			x1i = point[0];
			int x2i = point[1];
		...
		if (drawn && !seg.isSeen()) {
			...
			updateSeenCellsForSegment(seg); // updates seencells
		}
	}
	/**
	 ...
	 */
	private void updateSeenCellsForSegment(Seg seg) {
