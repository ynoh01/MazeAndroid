...
import edu.wm.cs.cs301.amazebyyunwoonoh.falstad.MazeConfiguration;
...
public class MapDrawer extends DefaultViewer {
	...
	public void redraw(MazePanel panel, StateGUI state, int px, int py,
		...
		if (null != controller) {
			if (controller.isInMapMode()) {
				draw_map(panel, px, py, walk_step, view_dx, view_dy, controller.isInShowMazeMode(), controller.isInShowSolutionMode()) ;
				draw_currentlocation(panel, view_dx, view_dy) ;
			}
		}
	 ...
	 */
	public void draw_map(MazePanel panel, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		// dimensions of the maze
		final int mazew = mazeConfig.getWidth() ;
		final int mazeh = mazeConfig.getHeight() ;
		
		panel.coloring("white");
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				if ((seencells.hasWall(x,y, CardinalDirection.North) || showMaze) && theCondition)
					...
					panel.drawLine(nx1, ny1, nx2, ny1);
				...
				if ((seencells.hasWall(x,y, CardinalDirection.West) || showMaze) && theCondition)
					panel.drawLine(nx1, ny1, nx1, ny2);
					//gc.drawLine(nx1, ny1, nx1, ny2);
			}
		
		if (showSolution) {
			draw_solution(panel, offx, offy, px, py) ;
		}
	}
	/**
	 ...
	 * @param //gc to draw on
	 */
	public void draw_currentlocation(MazePanel panel, int view_dx, int view_dy) {
		panel.coloring("red");
		...
		panel.fillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		//panel.ovalFilling(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		...
		// now the drawing
		panel.drawLine(ctrx, ctry, aptx, apty);
		//gc.drawLine(ctrx, ctry, aptx, apty);
		panel.drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
	 ...
	 * @param offy
	 */
	public void draw_solution(MazePanel panel, int offx, int offy, int px, int py) {
		...
		while (distance > 1) {
			...
			panel.drawLine(nx1, ny1, nx2, ny2);
